...
int	init_protected_boolean(struct s_protected_bool *protected_bool)
{
...
static int	initialize_philosopher(t_philo *philo, size_t philo_id,
		t_main_data data, pthread_mutex_t *left_fork)
{
	philo->id = philo_id;
...
static t_philo	*init_philo_tab(size_t number_of_philosophers,
		t_main_data data)
{
	t_philo			*philo_tab;
...
int	initialize_data(t_main_data *data)
{
	...
	data->philosophers = init_philo_tab(data->number_of_philosophers,
			*data);
